...
function wPlayerFrame(self)
	...
	PlayerStatusTexture:SetPoint("CENTER", self, "CENTER",16, 8);
	PlayerLevelText:ClearAllPoints();
	PlayerLevelText:SetPoint("CENTER", self, "CENTER", -100, 200);
...
whoaStatusFrame:SetScript('OnUpdate', function(self)
	...
	if TargetFrameNumericalThreat:IsVisible() then
		TargetFrameTextureFrameName:Hide();
	else
		TargetFrameTextureFrameName:Show();
	end
	if FocusFrameNumericalThreat:IsVisible() then
		FocusFrameTextureFrameName:Hide();
	else
